...
def list_sum(some_list):
    ...
    for i in range(len(some_list)):
        sum_list += some_list[i]
    return sum_list


list_example = [randint(1, 10) for i in range(10)]
print(list_example)
...
list_name = df["First name"]
list_age = df["Age"]
fig, ax = plt.subplots()
ax.plot(list_name, list_age)
ax.set_xlabel("Имена пользователей")
ax.set_ylabel("Возраст пользователей")
ax.set_title("График возраста пользователей")
plt.show()
